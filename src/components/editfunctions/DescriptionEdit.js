...
const DescriptionEdit = (props) => {
  ...
  const [value, setValue] = React.useState(props.character.description);
  ...
  const handleCheck = () => {
    ...
    submitableCharacter.description = value;
  ...
  const handleRandom = () => {
    ...
    try {
      fetch(
        `http://localhost:3000/api/v1/npc/${props.character.id}/description`,
        myInit
      )
        .then((res) => {
          return res.json();
  ...
  return (
    <div>
      <TextField value={value} multiline onChange={handleChange}></TextField>
      {loading ? (
        <CircularProgress />
      ) : (
        <Check
